/* ============================================================
   AIRPORTFITS — SPA JAVASCRIPT  (Medusa v2 Integration)
   ============================================================ */
...
const MEDUSA_PK    = 'pk_9014fcc2005bda2e9b2368162784daa8ff6e5fc988276a0649d568c3c7481154';
const REGION_ID    = 'reg_01KJT5F9PB8XSR1VJZT92CCGBF';   // International / USD
...
// Populated from Medusa on load — same shape the old hard-coded list had
let PRODUCTS = [];
// Raw Medusa product objects (keyed by handle)
let medusaProducts = {};
// Medusa cart state
...
/** Fetch all products from Medusa and build the flat PRODUCTS list */
async function loadProducts() {
  try {
    const data = await medusaFetch(
      `/store/products?region_id=${REGION_ID}&fields=+metadata,*variants,*variants.options,*variants.calculated_price&limit=50`
    );
    ...
    for (const p of data.products || []) {
      medusaProducts[p.handle] = p;

      const design  = (p.metadata && p.metadata.design)  || p.title.replace(' Hoodie & Jogger Set', '');
      const gender  = (p.metadata && p.metadata.gender)   || 'men';
      const subtitle = p.subtitle || '';

      // Find all distinct colors in this product's variants
      const colorsInProduct = [];
      for (const v of p.variants || []) {
        const colorOpt = (v.options || []).find(o =>
          o.option && o.option.title === 'Color'
        );
        const colorName = colorOpt ? colorOpt.value : null;
        if (colorName && !colorsInProduct.includes(colorName)) {
          colorsInProduct.push(colorName);
        }
      }

      // One display-product entry per color (backward-compatible with old layout)
      for (const color of colorsInProduct) {
        const cm = COLOR_MAP[color];
        // Pick the first variant for this color to get price
        const sampleVariant = (p.variants || []).find(v =>
          (v.options || []).some(o => o.value === color)
        );
        const price = sampleVariant?.calculated_price?.calculated_amount ?? 0;
        ...
        PRODUCTS.push({
          ...
          color,
          price,
          img:           cm ? cm.img : './assets/product-black-set.png',
          // Store variant id for the default size so addToBag can resolve it
          _variants:     (p.variants || []).filter(v =>
            (v.options || []).some(o => o.value === color)
          ),
        });
      }
    ...
    console.error('[AirportFits] Failed to load products from Medusa:', err);
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  // Load products from Medusa before anything else
  await loadProducts();
  initRouter();
  initHeader();
  initMobileMenu();
...
function addToBag() {
  ...
  // Resolve the Medusa variant that matches selected color + size
  const variant = (product._variants || []).find(v =>
  ...
  if (existingIdx > -1) {
    ...
    cart.push({
      ...
      medusaProductId: product.medusaId,
      variantId,
...
async function placeOrder() {
  ...
  try {
    // Attempt to create a Medusa cart and add line items
    ...
    try {
      ...
      console.warn('[AirportFits] Medusa cart creation skipped:', cartErr);
    }

    // Generate local order confirmation (Medusa payment not yet wired)
    lastOrder = {
    ...
    console.error('[AirportFits] Order error:', err);
    showToast('Something went wrong — please try again', 'size');
  }
...
function initNewsletterForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // TODO: Wire to Medusa customer / newsletter module when ready
